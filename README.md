...
| Property edits are coalesced into one undo step per burst, not one per keystroke                       | `updateSelectedField` fires on every keystroke, so recording history unconditionally would make undo revert one character at a time. `stores/history.ts` folds same-key `record()` calls within a short window into the entry already on top of the stack; a pause longer than that starts a new step.                                    |
| `parseJsonSchema` (`fields/schema.ts`) is the inverse of `generateJsonSchema`, also a pure function    | Same testability/reuse rationale as `generateJsonSchema`. Import is best-effort: schema data the builder doesn't model (unsupported types, keywords, malformed `allOf` conditions) is skipped and collected into a report instead of rejecting the whole file, and only throws when there's nothing recognizable to import at all.        |
